...
Options parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (consume_arg(i, argc, argv, "--device", value)) {
            opt.device = std::stoi(value);
    ...
    if (opt.chi <= 0 || opt.maxiter < 0 || opt.miniter < 0 ||
        opt.miniter > opt.maxiter || opt.arnoldi_tol < 0.0 || opt.tol < 0.0 ||
        opt.arnoldi_restarts < 0 || opt.init_relax < 0 ||
        opt.init_relax_arnoldi_tol < 0.0 ||
        opt.device < 0 || opt.warmup < 0 || opt.repetitions <= 0) {
        throw std::invalid_argument("invalid dimensions, device, or tolerances");
    }
...
LoadedState load_state(const std::string &path, int64_t expected_chi) {
    ...
    (void)read_scalar<double>(in);
    (void)read_scalar<int64_t>(in);
    (void)read_scalar<int>(in);
...
double *device_alloc_and_copy(const std::vector<double> &host) {
    double *ptr = nullptr;
    cuda_check(cudaMalloc(reinterpret_cast<void **>(&ptr),
                          host.size() * sizeof(double)),
               "cudaMalloc");
    try {
        cuda_check(cudaMemcpy(ptr, host.data(), host.size() * sizeof(double),
...
void copy_host_to_device(const std::vector<double> &host, double *device) {
    cuda_check(cudaMemcpy(device, host.data(), host.size() * sizeof(double),
                          cudaMemcpyHostToDevice),
               "cudaMemcpy host to device");
}

void copy_device_to_host(double *device, std::vector<double> &host) {
    cuda_check(cudaMemcpy(host.data(), device, host.size() * sizeof(double),
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        dM = device_alloc_and_copy(M);
        dAL = device_alloc_and_copy(AL);
        dAR = device_alloc_and_copy(AR);
        dC = device_alloc_and_copy(C);
        dFL = device_alloc_and_copy(FL);
        dFR = device_alloc_and_copy(FR);

        double err = 0.0;
        int64_t iterations = 0;
        int converged = 0;
        auto reset_state = [&]() {
            copy_host_to_device(AL0, dAL);
            copy_host_to_device(AR0, dAR);
            copy_host_to_device(C0, dC);
            copy_host_to_device(FL0, dFL);
            copy_host_to_device(FR0, dFR);
        };
        auto run_once = [&]() {
            return tenet_native_ising_vumps_run_d_cuda(
                opt.chi, phys, dM, dAL, dAR, dC, dFL, dFR, opt.max_k,
                opt.arnoldi_tol, opt.tol, opt.miniter, opt.maxiter, &err,
                &iterations, &converged);
        };
        auto report_status = [](int status) {
            std::cerr << "native cuda status=" << status << " ("
                      << tenet_native_status_string(status)
                      << "): " << tenet_native_last_error() << "\n";
        };
        ...
        copy_device_to_host(dAL, AL);
        copy_device_to_host(dAR, AR);
        copy_device_to_host(dC, C);
        copy_device_to_host(dFL, FL);
        copy_device_to_host(dFR, FR);
        ...
        std::cout << std::setprecision(17)
                  << "status=success backend=cuda"
                  << " device=" << opt.device << " device_name=" << prop.name
